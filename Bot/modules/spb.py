...
@app.on_message(filters.command("spb", ".") & filters.me)
async def lookup(client, message):
    ...
    try:
        ...
        await message.edit_text("I don't know that User.")
        sleep(2)
        return
    ...
    if response == True:
        ...
        await message.edit_text(stats, disable_web_page_preview=True)
    else:
        await message.edit_text("`cannot reach SpamProtection API`")
        await sleep(3)
